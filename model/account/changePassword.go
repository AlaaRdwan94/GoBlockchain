...
func ForgetPassword(w http.ResponseWriter, req *http.Request) {
	...
	if accountObjbyEmail.AccountPublicKey != "" && contact.Email != "" {
		...
		` +
			validator.CurrentValidator.ValidatorIP + "/" + RP.PathApi +
			`
		...
		globalPkg.WriteLog(logobj, "success send confirmation code"+Confirmation_code, "success")
		return
	}
	...
	globalPkg.WriteLog(logobj, "invalid Email Or Phone"+Confirmation_code, "failed")
