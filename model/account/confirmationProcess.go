...
func createPublicAndPrivate(userobj User) User {
	...
	for {
		...
		pk2 := []byte(userobj.Account.AccountPublicKey)
		...
		fmt.Println("----- savePKObj", savePKObj)
		...
		if accountobj.AccountPublicKey == "" { //not found this public key in account DB
			...
			accountPublicKey := cryptogrpghy.GetPublicPEMKey(key.PublicKey)
			// pk2 return pubKey as byte
			// pk2 := cryptogrpghy.GetPublicPEMKeybyte(key.PublicKey)
			pk2 = []byte(accountPublicKey)
			add = cryptogrpghy.Address(pk2)
			userobj.Account.AccountPublicKey = string(add)

			// save pk, address
			savePKObj.Publickey = accountPublicKey
			savePKObj.Address = string(add)
			fmt.Println("----- savePKObj", savePKObj)
			// SavePKAddress(savePKObj)
			broadcastTcp.BoardcastingTCP(savePKObj, "", "savepk")
