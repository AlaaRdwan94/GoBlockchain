...
func Login(w http.ResponseWriter, req *http.Request) {
	...
	//if accountObj.AccountPublicKey == "" && accountObj.AccountName != "" {
	if (accountObj.AccountPublicKey == "" || (!accountObj.AccountStatus && accountObj.AccountPublicKey != "")) && accountObj.AccountName != "" {
		var user User
...
func SavePublickey(w http.ResponseWriter, req *http.Request) {
	...
	account := getAccountByEmail(saveKeyReq.Email) //Denta
	isFine := true
	if account.AccountPassword != saveKeyReq.Passsword {
		isFine = false
		errStr = errorpk.AddError("SavePublickey", "wrong password!", "hack")
	} else if account.AccountEmail != saveKeyReq.Email {
		isFine = false
		errStr = errorpk.AddError("SavePublickey", "wrong email!", "hack")
	} else if account.AccountPublicKey != saveKeyReq.PublicKey {
		isFine = false
		errStr = errorpk.AddError("SavePublickey", "wrong public key", "hack")
	} else if account.Confirmed {
		isFine = false
		errStr = errorpk.AddError("SavePublickey", "existing publickey", "hack")
	}
	...
	account.Confirmed = true     // Denta
	account.AccountStatus = true // Denta
