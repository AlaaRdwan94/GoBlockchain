...
func AllStats(w http.ResponseWriter, r *http.Request) {
	...
	networkData := map[string]interface{}{
		"host name":                       hostname,
		"node established requests count": len(connections),
		"network interfaces stats":        networkStatsOutput,
	}
	...
	utils.Respond(w, http.StatusOK, map[string]interface{}{
		"node cpu usage":             fmt.Sprintf("%.2f%%", cpuServerUsage),
		"node memory usage":          utils.FormatBytes(memUsage, 2),
		"network stats":              networkData,
		"storage stats":              storageStatsOutput,
		"current transactions count": 20, // new_transaction.CountTransaction(),
	})
}
...
func NetworkStats(w http.ResponseWriter, r *http.Request) {
	...
	utils.Respond(w, http.StatusOK, map[string]interface{}{
		"host name":                       hostname,
		"node established requests count": len(connections),
		"network stats":                   networkStatsOutput,
	})
}
...
func StorageStats(w http.ResponseWriter, r *http.Request) {
	...
	// return the response for the client
	utils.Respond(w, http.StatusOK, map[string]interface{}{
		"partitions stats": storageStatsOutput,
	})
}
