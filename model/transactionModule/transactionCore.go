...
func GetUnspentAndSpentTxs(publicKey string) ([]transaction.TXInput, []transaction.TXInput) {
	...
	for _, blockObj := range accountObj.BlocksLst {
		blockObj = cryptogrpghy.KeyDecrypt(validator.CurrentValidator.ValidatorPrivateKey, blockObj)
		...
		if isAddingCoinsToIno {
			...
			if outputSum == inputSum {
				...
				if !inputexist {
					...
					fmt.Println("transaction.PendingValidationTxs[senderPK]", transaction.PendingValidationTxs[senderPK])
					fmt.Println("tx.TransactionInput", tx.TransactionInput)
