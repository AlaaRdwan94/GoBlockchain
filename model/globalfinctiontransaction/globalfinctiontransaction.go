	...
	if len(blocktransactionslst) != 0 {
		for i := len(blocktransactionslst) - 1; i >= 0; i-- {
			...
			if transactionid != "" && blocktransactionslst[i].Validator == validator.CurrentValidator.ValidatorIP {
				...
				// transactionInputlst := blocktransactionslst[i].TransactionInput
				// if transactionInputlst != nil && blocktransactionslst[i].Validator == validator.CurrentValidator.ValidatorIP {
				// 	lastTransactionInput := transactionInputlst[len(transactionInputlst)-1]
				// 	lastTransactionInputID := lastTransactionInput.InputID
				// 	InputIDArray := strings.Split(lastTransactionInputID, "_")
				// 	if InputIDArray[0] == CreateValidatorHash(validator.CurrentValidator.ValidatorIP) {
				// 		lastindex, _ = ConvertIntToFixedLengthString(ConvertFixedLengthStringtoInt(InputIDArray[1])+1, 5)
