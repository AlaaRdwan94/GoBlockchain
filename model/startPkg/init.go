...
func Init() {
	...
	if len(accountdb.GetAllAccounts()) == 0 {
		...
		if Conf.Server.FirstMiner {
			...
			validator.CurrentValidator = validator.ValidatorStruct{"http://" + Conf.Server.Ip + ":" + Conf.Server.Port, Conf.Server.PrivIP + ":" + Conf.Server.SoketPort, string(dat1), string(dat2), Conf.Server.InitialStakeCoin, now, true, now, false, ""}
			validator.AddValidator(validator.ValidatorStruct{"http://" + Conf.Server.Ip + ":" + Conf.Server.Port, Conf.Server.Ip + ":" + Conf.Server.SoketPort, validator.CurrentValidator.ValidatorPublicKey, validator.CurrentValidator.ValidatorPrivateKey, Conf.Server.InitialStakeCoin, now, true, now, false, ""})
			InitTheaccount()
			// fmt.Println("validator.CurrentValidator after : ", validator.CurrentValidator)

		} else {
			validator.CurrentValidator = validator.ValidatorStruct{"http://" + Conf.Server.Ip + ":" + Conf.Server.Port, Conf.Server.PrivIP + ":" + Conf.Server.SoketPort, string(dat1), string(dat2), Conf.Server.InitialStakeCoin, now, true, now, false, validator.NewIndex()}
